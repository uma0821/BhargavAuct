...
export async function GET(req) {
   ...
   const sortColumn = url.searchParams.get('sortColumn') || 'mat_id'; // Default to 'id' if not present
   ...
   const allowedColumns = ['mat_id', 'mat_name', 'applied_tax'];
   ...
   try {
       ...
       const query = `
           ...
           WHERE mat_name LIKE ?  -- Search for material by name
       ...
       const totalQuery = `
           SELECT COUNT(*) as total FROM material WHERE mat_name LIKE ?
       `;
       ...
       console.error("Error in material API call:", error);
       return NextResponse.json(
