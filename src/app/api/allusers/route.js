...
export async function GET(req) {
    ...
    const sortColumn = url.searchParams.get('sortColumn') || 'id'; // Default to 'id' if not present
    ...
    const allowedColumns = ['id', 'name', 'email', 'address', 'user_status', 'bid_limit'];
    ...
    try {
        ...
        const query = `
            SELECT * FROM user
            WHERE name LIKE ?  -- Search for users by name
        ...
        console.error("Error in user API call:", error);
        return NextResponse.json(
